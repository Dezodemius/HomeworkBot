...
namespace HomeWorkTelegramBot.Core
{
  internal static class AnswerService
  {
    ...
    /// <summary>
    /// Получает ответ для указанного пользователя и задания и логирует это действие.
    /// </summary>
    /// <param name="chatId">Идентификатор чата пользователя.</param>
    /// <param name="taskId">Идентификатор задания.</param>
    /// <returns>Ответ для пользователя и задания или null, если не найден.</returns>
    public static Answer GetAnswerByChatIdAndTaskId(long chatId, int taskId)
    {
      var answer = answerRepository.GetAnswerByChatIdAndTaskId(chatId, taskId);
      if (answer != null)
      {
        LogInformation($"Найден ответ для пользователя с ChatId {chatId} и задания с Id {taskId}");
      }
      else
      {
        LogWarning($"Ответ для пользователя с ChatId {chatId} и задания с Id {taskId} не найден.");
      }
      return answer;
    }
  }
